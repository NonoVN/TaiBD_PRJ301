...
public class AddUniversityControlller extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            UniversityDTO user = udao.searchByID(id);
            if (user != null) {
            ...
            try {
                ...
                if (foundedYear < 0) {
                    error += ("So hoc sinh phai la so nguyen duong!<br/>");
                }
            } catch (Exception e) {
                error += ("So hoc sinh phai la so nguyen!<br/>");
            }
            ...
            try {
                ...
                if (foundedYear < 0) {
                    error += ("So giang vien phai la so nguyen duong!<br/>");
                }
            } catch (Exception e) {
                error += ("So giang vien phai la so nguyen!<br/>");
            }

            boolean isDraft = (s_isDraft.equals("1")) ? true : false;

            user = new UniversityDTO(id, name, shortName, description, foundedYear, address, city, region, type, totalStudents, totalFaculties, isDraft);
